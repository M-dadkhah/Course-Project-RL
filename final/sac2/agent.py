...
class Agent(object):
	...
	def __init__(
		...
		self.replay_buffer = ReplayBuffer(int(5e4))  # share the replay buffer through manager

		self.soft_q_net1 = SoftQNetwork(self.obs_space, self.act_space, hidden_dim).to(device)
		self.soft_q_net2 = SoftQNetwork(self.obs_space, self.act_space, hidden_dim).to(device)
		self.target_soft_q_net1 = SoftQNetwork(self.obs_space, self.act_space, hidden_dim).to(device)
		self.target_soft_q_net2 = SoftQNetwork(self.obs_space, self.act_space, hidden_dim).to(device)
		self.policy_net = PolicyNetwork(self.obs_space, self.act_space, hidden_dim, self.action_range).to(device)
	...
	def act(self, state, mode='eval'):
		...
		if mode=='train':
			...
			action = action + np.random.normal(0, self.expl_noise, size=self.act_space)
		return action
	...
	def update(self, curr_obs, action, reward, next_obs, done, timestep, batch_size=int(2**7)):
		...
		# Training Q Function
			...
			target_q_value = reward + (1 - done) * self.discount * target_q_min # if done==1, only reward
	...
	def save(self, root_path):		
		...
		torch.save(self.soft_q_net1.state_dict(), root_path+'_q1')
		torch.save(self.soft_q_net2.state_dict(), root_path+'_q2')
		torch.save(self.policy_net.state_dict(), root_path+'_policy')

	def load_weights(self, root_path):
		try:
			
			self.soft_q_net1.load_state_dict(torch.load(root_path+'_q1'))
			self.soft_q_net2.load_state_dict(torch.load(root_path+'_q2'))
