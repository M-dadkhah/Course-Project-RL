...
class Agent(object):
	...
	def act(self, curr_obs, mode='eval'):
		if mode=='train': # adding some noise!
			if  self.timestep<self.start_timesteps: # warming up with random actions
				return self.act_space.sample()
			else: # actions adding a noise with a decaying variance for more observation at first 
				curr_obs = torch.FloatTensor(curr_obs.reshape(1, -1)).to(device)
				action = self.actor(curr_obs).cpu().data.numpy().flatten()

				self.expl_noise = 0.5*(1-np.tanh(self.timestep/1.3e5))
				return (action \
					+ np.random.normal(0, 1 * self.expl_noise, size=self.act_space.shape[0])
					).clip(-1, 1)
		elif mode=='eval': # actions without any noise and for evaluation
			curr_obs = torch.FloatTensor(curr_obs.reshape(1, -1)).to(device)
			action = self.actor(curr_obs).cpu().data.numpy().flatten()
			return action
