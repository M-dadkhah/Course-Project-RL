...
class Agent(object):
	...
	def __init__(
		...
		self.total_it = 0
		self.acts=[self.act_space.sample() for _ in range(3)]
	...
	# else:
		self.acts.insert(0, self.acts[0]*.1 + self.acts[1]*.15 + self.acts[2]*.2 + .55*self.actor(curr_obs).cpu().data.numpy().flatten())
		self.acts.pop()
		return self.acts[0]
